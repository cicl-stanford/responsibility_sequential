...
single_experiment_file={PLACEHOLDER}/code/bash/generate_counterfactual_decisions_slurm.sh
...
sim_scalers=(0.6 0.8 1.0 1.2 1.4 1.6 1.8 2.0)
...
for world_idx in ${!world_order[@]}; do
    ...
    for sim_scaler in ${sim_scalers[@]}; do
        # submit the job
        sbatch -c 1 -o slurmcf:${world_name}_simscaler:${sim_scaler}.out -e slurmcf:${world_name}_simscaler:${sim_scaler}.err --mem=$job_mem --time=$job_time --partition=$job_partition $single_experiment_file $world_name $trial_name $sim_scaler
    done
done